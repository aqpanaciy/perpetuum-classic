...
namespace Perpetuum.Services.TechTree
{
    public class TechTreeService : ITechTreeService
    {
        ...
        public IEnumerable<TechTreeNode> GetUnlockedNodes(long owner)
        {
            ...
            return Db.Query().CommandText("select definition from techtreeunlockednodes where owner = @owner").SetParameter("@owner",owner).Execute().Select(r => nodes[r.GetValue<int>(0)]).ToArray();
        }
